...
Error anvil_memory_alloc_lazy(void** mem_out, const u64 capacity, const u64 alignment) noexcept {
        ...
        if (capacity >= (1UL << 21)) { // NOLINT enable huge page advice for allocations larger than 2MB
...
Error anvil_memory_alloc_eager(void** mem_out, const u64 capacity, const u64 alignment) noexcept {
        REQUIRE(mem_out != nullptr, NULL_PARAMETER);
        REQUIRE(*mem_out == nullptr, NULL_PARAMETER);
        REQUIRE(capacity > 0, INVALID_ARGUMENTS);
        REQUIRE(capacity <= MAX_CAPACITY, INVALID_ARGUMENTS);
...
Error anvil_memory_commit(void* ptr, const u64 commit_size) noexcept {
        REQUIRE(ptr != nullptr, NULL_PARAMETER);
        ...
        const u64 COMMIT_SIZE = (commit_size + (PAGE_SIZE - 1)) & ~(PAGE_SIZE - 1);
