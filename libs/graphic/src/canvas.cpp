...
anvil::Error anvil::graphic::create(Canvas* const canvas_out, const u64 width, const u64 height, const char fill_char = ' ') {
        ...
        REQUIRE(width - 1 < MAX_WIDTH, anvil::INVALID_DIMENSIONS);
        ...
        if (!canvas_out->buffer) {
                ...
                INVARIANT(CLEANUP_ERROR == anvil::OK);
                return anvil::OUT_OF_MEMORY;
        }
...
anvil::Error anvil::graphic::set(Canvas* const canvas_out, const u64 index, const char character) {
        REQUIRE(canvas_out != nullptr, anvil::NULL_PARAMETER);
        REQUIRE(canvas_out->buffer != nullptr, anvil::NULL_PARAMETER);
        REQUIRE(index < (canvas_out->width * canvas_out->height), anvil::INVALID_ARGUMENTS);

        canvas_out->buffer[index] = character;
        return anvil::OK;
}

anvil::Error anvil::graphic::destroy(Canvas* const canvas_out) {
        REQUIRE(canvas_out != nullptr, anvil::NULL_PARAMETER);
        INVARIANT(canvas_out->buffer != nullptr);
        INVARIANT(canvas_out->allocator != nullptr);

        INVARIANT(anvil::memory::destroy(&canvas_out->allocator) == anvil::OK);
        canvas_out->height    = 0;
        canvas_out->width     = 0;
